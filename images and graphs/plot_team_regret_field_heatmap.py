...
def compute_team_grid(df: pd.DataFrame, x_edges: np.ndarray, y_edges: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    # 2D histograms on (y, x) so returned matrices align with pcolormesh(x_edges, y_edges, Z).
    counts, _, _ = np.histogram2d(df["carrier_y"], df["carrier_x"], bins=[y_edges, x_edges])
    sums, _, _ = np.histogram2d(df["carrier_y"], df["carrier_x"], bins=[y_edges, x_edges], weights=df["suboptimality"])
            ...
            rows.append(
                {
                    ...
                    "mean_regret": float(mean[yi, xi]) if np.isfinite(mean[yi, xi]) else np.nan,
                }
            )
...
def main():
    parser = argparse.ArgumentParser(description="Plot team regret by field location using square bins.")
    parser.add_argument("--rows", default="outputs/player_suboptimality_possessions.csv", help="Input possession rows CSV")
    ...
    parser.add_argument("--min-count", type=int, default=1, help="Minimum actions in a bin to display a value")
    parser.add_argument("--output", default="outputs/team_regret_field_heatmap.png", help="Output PNG path")
    parser.add_argument("--output-grid", default="outputs/team_regret_field_heatmap_grid.csv", help="Output CSV with bin summaries")
    ...
    needed = ["team_side", "carrier_x", "carrier_y", "suboptimality"]
    ...
    df["suboptimality"] = pd.to_numeric(df["suboptimality"], errors="coerce")

    df = df[
        df["team_side"].isin(["home", "away"])
        & np.isfinite(df["carrier_x"])
        & np.isfinite(df["carrier_y"])
        & np.isfinite(df["suboptimality"])
    ].copy()

    if df.empty:
        raise ValueError("No finite rows with team_side, carrier location, and suboptimality.")
    ...
    home_mean, home_counts = compute_team_grid(home_df, x_edges, y_edges)
    away_mean, away_counts = compute_team_grid(away_df, x_edges, y_edges)
    ...
    all_vals = np.concatenate([
        home_mean[np.isfinite(home_mean)],
        away_mean[np.isfinite(away_mean)],
    ])
    if all_vals.size == 0:
    ...
    # Reserve headroom for title + subtitle.
    fig.subplots_adjust(top=0.76, wspace=0.08)

    im0 = axes[0].pcolormesh(x_edges, y_edges, home_mean, cmap="YlOrRd", vmin=vmin, vmax=vmax)
    axes[0].set_title(f"{ctx['home_name']} Avg Regret by Location")
    draw_pitch_overlay(axes[0], half_length, half_width)
    axes[0].set_xlabel("x (m)")
    axes[0].set_ylabel("y (m)")
    axes[0].set_aspect("equal", adjustable="box")

    axes[1].pcolormesh(x_edges, y_edges, away_mean, cmap="YlOrRd", vmin=vmin, vmax=vmax)
    axes[1].set_title(f"{ctx['away_name']} Avg Regret by Location")
    ...
    cbar.set_label("Average Suboptimality (Optimal - Taken)")

    fig.suptitle("Team Regret Heatmaps by Field Location", fontsize=14, y=0.95)
